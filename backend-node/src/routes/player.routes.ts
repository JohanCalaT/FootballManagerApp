...
router.post(
  ...
  body('league')        .isString().trim().isLength({ min: 1, max: 100 })
                        .withMessage('debe ser string [1, 100]'),
  body('firstName')     .optional({ values: 'null' }).isString().isLength({ max: 100 }),
  body('lastName')      .optional({ values: 'null' }).isString().isLength({ max: 100 }),
  body('nationality')   .optional({ values: 'null' }).isString().isLength({ max: 100 }),
  body('birthDate')     .optional({ values: 'null' }).isISO8601().withMessage('debe ser ISO 8601'),
  body('birthPlace')    .optional({ values: 'null' }).isString().isLength({ max: 100 }),
  body('birthCountry')  .optional({ values: 'null' }).isString().isLength({ max: 100 }),
  body('height')        .optional({ values: 'null' }).isString().isLength({ max: 20 }),
  body('weight')        .optional({ values: 'null' }).isString().isLength({ max: 20 }),
  body('injured')       .optional({ values: 'null' }).isBoolean().toBoolean(),
  body('position')      .optional({ values: 'null' }).isIn(POSITIONS)
                        .withMessage(`debe estar en ${POSITIONS.join('|')}`),
  body('shirtNumber')   .optional({ values: 'null' }).isInt({ min: 1, max: 99 }).toInt(),
  body('imageUrl')      .optional({ values: 'null' }).isString().isLength({ max: 500 }),
  body('imageSource')   .optional({ values: 'null' }).isIn(IMAGE_SOURCES)
                        .withMessage(`debe estar en ${IMAGE_SOURCES.join('|')}`),
  body('apiFootballId') .optional({ values: 'null' }).isInt({ min: 1 }).toInt(),
  runValidations,
...
router.put(
  '/:id',
  requireAdmin,
  body('name')          .optional({ values: 'null' }).isString().trim().isLength({ min: 1, max: 100 }),
  body('team')          .optional({ values: 'null' }).isString().trim().isLength({ min: 1, max: 100 }),
  body('league')        .optional({ values: 'null' }).isString().trim().isLength({ min: 1, max: 100 }),
  body('firstName')     .optional({ values: 'null' }).isString().isLength({ max: 100 }),
  body('lastName')      .optional({ values: 'null' }).isString().isLength({ max: 100 }),
  body('nationality')   .optional({ values: 'null' }).isString().isLength({ max: 100 }),
  body('birthDate')     .optional({ values: 'null' }).isISO8601(),
  body('birthPlace')    .optional({ values: 'null' }).isString().isLength({ max: 100 }),
  body('birthCountry')  .optional({ values: 'null' }).isString().isLength({ max: 100 }),
  body('height')        .optional({ values: 'null' }).isString().isLength({ max: 20 }),
  body('weight')        .optional({ values: 'null' }).isString().isLength({ max: 20 }),
  ...
  body('imageUrl')      .optional({ values: 'null' }).isString().isLength({ max: 500 }),
  body('imageSource')   .optional({ values: 'null' }).isIn(IMAGE_SOURCES),
  body('playerGeolocation.lat').optional({ values: 'null' }).isFloat({ min: -90,  max: 90  }),
  body('playerGeolocation.lng').optional({ values: 'null' }).isFloat({ min: -180, max: 180 }),
  body('playerGeolocation.city')   .optional({ values: 'null' }).isString().isLength({ max: 100 }),
  body('playerGeolocation.country').optional({ values: 'null' }).isString().isLength({ max: 100 }),
  runValidations,
